...
void main() async {
  ...
  test('test ftpConnect', () async {
    ...
    final FTPConnect ftpConnectNoLog = FTPConnect(
      "users.on.net",
      user: "pvpt",
      pass: "Lachdhaf",
      securityType: SecurityType.ftpes,
    ...
    expect(
      () async => await ftpConnectNoLog.connect(),
  ...
  test('test ftpConnect No log', () async {
    ...
    expect(await ftpConnectNoLog.connect(), equals(true));
    await ftpConnectNoLog.currentDirectory();
    ftpConnectNoLog.listCommand = ListCommand.list;
    ftpConnectNoLog.supportIPV6 = false;
    await ftpConnectNoLog.setTransferType(TransferType.binary);
    await ftpConnectNoLog.listDirectoryContent();
    await ftpConnectNoLog.setTransferType(TransferType.ascii);
    ftpConnectNoLog.transferMode = TransferMode.passive;
    await ftpConnectNoLog.listDirectoryContent();

    expect(await ftpConnectNoLog.disconnect(), equals(true));
  });

  test('test ftpConnect timeOut', () async {
    final FTPConnect ftpConnectTimeOut = FTPConnect(
      "speedtest.tele2.net",
      user: "xxxcx",
      pass: "xxxx",
      securityType: SecurityType.ftpes,
    ...
    expect(
      () async => await ftpConnectTimeOut.connect(),
      throwsA(isA<FTPConnectionTimeoutException>()),
    );
  });

  test('test ftpConnect error connect', () async {
    FTPConnect ftpConnectErrorConnect = FTPConnect(
      "demo.wftpserver.com",
      user: "xxxx",
      pass: "xxxx",
    );
  ...
  test('test ftpConnect Dir functions', () async {
    ...
    expect(
      await ftpConnect.checkFolderExistence("dirName${DateTime.now()}"),
      equals(false),
    );
  ...
  test('test ftpConnect File functions', () async {
    ...
    expect(
      await ftpConnect.uploadFile(
        await fileMock(fileName: fileName),
    ...
    expect(
      await ftpConnect.downloadFile(
        fileName,
        File('$testFileDir/$localDownloadFile'),
    ...
    expect(
      await ftpConnect.rename(fileName, '${fileName}_renamed.txt'),
      equals(true),
    );

    //test delete file (false because the server is protected)
    expect(
      await ftpConnect.deleteFile('${fileName}_renamed.txt'),
      equals(true),
    );
  ...
  test('test FTP Entry Class', () {
    ...
    expect(
      () => FTPEntry.parse(data4, ListCommand.list),
      throwsA(isA<FTPParsingErrorException>()),
    );

    String data5 = "";
    expect(
      () => FTPEntry.parse(data5, ListCommand.mlsd),
      throwsA(isA<FTPParsingErrorException>()),
    );
    expect(
      () => FTPEntry.parse(data5, ListCommand.list),
      throwsA(isA<FTPParsingErrorException>()),
    );
  });
